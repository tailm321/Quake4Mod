...
void idAI::AdjustHealthByDamage	( int damage ) {
	...
	//ugly way of getting grenade launcher to give health on doing damage, how it does damage can be set in def file
	idPlayer* player = gameLocal.GetLocalPlayer();
	if (player->GetCurrentWeapon() == 4){
...
void idAI::OnDeath( void ){
	...
	//uncommenting this section to enable enemies dropping loot
	...
	if( spawnArgs.GetFloat( "no_drops" ) >= 1.0 ){
		...
		if (rVal < 20){
			...
			spawnArgs.Set("def_dropsItem1", "item_arnor_large");
		}
		else if (rVal < 42){
