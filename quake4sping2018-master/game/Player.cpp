...
// RITUAL END
	...
	//loop to constantly regenerate machinegun ammo
	if (!gameLocal.isMultiplayer){
		int k = 1;
		int ammoIndex = inventory.AmmoIndexForWeaponIndex(k);
		int max = inventory.StartingAmmoForWeaponIndex(k);
		while (gameLocal.time > nextAmmoRegenPulse[ammoIndex] && inventory.ammo[ammoIndex] < max){
			
			int step = inventory.AmmoRegenStepForWeaponIndex(k);
			int time = inventory.AmmoRegenTimeForWeaponIndex(k);
...
// RAVEN END
	...
	//repurposing the ammoregen to constantly regenerate a specific weapon's ammo
	//since multiple weapons will use the same ammo type, it will apply to all affected weapons, emmulating "mana"

	//weapon whose ammo will be affected, 8 should be lightning gun
	int manaWeapon = 8;
	int shoutWeapon = 9;
	int manaIndex = inventory.AmmoIndexForWeaponIndex(manaWeapon);
	int shoutIndex = inventory.AmmoIndexForWeaponIndex(shoutWeapon);
	//the max ammo to be regenerated up to, hard set to 400 as that matches the number set for maxammo in the "spell" and shouldn't change from that
	int maxMana = 400;
	int maxShout = 100;

	if (gameLocal.time > nextAmmoRegenPulse[manaWeapon] && inventory.ammo[manaIndex] < maxMana){
		int step = inventory.AmmoRegenStepForWeaponIndex(manaWeapon);
		int time = inventory.AmmoRegenTimeForWeaponIndex(manaWeapon);

		if (inventory.ammo[manaWeapon] < maxMana){
			inventory.ammo[manaWeapon] += 1;
		}
		if (inventory.ammo[manaWeapon] >= maxMana){
			inventory.ammo[manaWeapon] = maxMana;
		}

		nextAmmoRegenPulse[manaWeapon] = gameLocal.time + time;
	}

	//doing the same but regenerating shout timer
	if (gameLocal.time > nextAmmoRegenPulse[shoutWeapon] && inventory.ammo[shoutIndex] < maxShout){
		int step = inventory.AmmoRegenStepForWeaponIndex(shoutWeapon);
		int time = inventory.AmmoRegenTimeForWeaponIndex(shoutWeapon);

		if (inventory.ammo[shoutIndex] < maxShout){
			inventory.ammo[shoutIndex] += 1;
		}
		if (inventory.ammo[shoutIndex] >= maxShout){
			inventory.ammo[shoutIndex] = maxShout;
		}

		nextAmmoRegenPulse[shoutIndex] = gameLocal.time + time;
	}
